...
for i in {0..100}
do 
    make JOBS=${i} WORK_PER_JOB=100 clean serial && ./serial >> serial.csv
done 

for i in {0..100}
do 
    make JOBS=${i} WORK_PER_JOB=100 clean pthreads && ./pthreads >> pthreads.csv
done 

for i in {0..100}
do 
    make JOBS=${i} WORK_PER_JOB=100 clean libdispatch && ./libdispatch >> libdispatch.csv
done 
